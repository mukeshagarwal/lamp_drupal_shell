...
sh ./project-setup.sh $project_url $branch $project_branch $project_dir $db_name $db_user $db_pass $method $files_url $db_url $project_remote_ip $project_remote_user $secure_key
# RUn script for setting post fix mail server
sh ./mailserver-postfix.sh

# Optimize server by installing apc and memcache
sh ./server-optimize.sh $project_dir
